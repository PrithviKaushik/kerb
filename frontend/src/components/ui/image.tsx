...
/* eslint-disable @next/next/no-img-element, jsx-a11y/alt-text -- custom base44/Wix image pipeline: raw <img> is intentional (own WebP/resize optimization) and alt text arrives via props spread. */
...
>(
  (
    ...
    const [previewSource, setPreviewSource] =
      React.useState<PreviewSource | null>(null);

    const preview =
      previewSource?.source === sourceUrl
        ? previewSource
        : null;
    ...
    const handleError = (
      ...
      if (
        nextMode === IMAGE_LOAD_MODE.FALLBACK
      ) {
        onError?.(event);
      }
    };
    ...
    > = {
        ...
        className:
          preview &&
          preview.sourceClassName === props.className
            ? preview.className
            : props.className,
        onError: handleError,
      };
    ...
    if (!src) {
      ...
      // tag being `img`, so a placeholder div would be unrecoverable in
      // the editor. FALLBACK_IMAGE_URL doubles as the "no image chosen" graphic.
      return (
        <img
          ref={ref}
